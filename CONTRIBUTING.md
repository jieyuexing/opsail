...
crates/opsail-usage    Remaining-usage queries for supported CLI providers
...
`opsail-usage` exposes `read_usage(&UsageOptions)`, returning the versioned `UsageReport` used by CLI JSON output. An empty provider list queries every registered provider; Codex and Grok are currently registered. Each adapter owns its discovery and query protocol. Reports must not expose credentials or raw provider responses. An unavailable provider does not block the remaining rows.
